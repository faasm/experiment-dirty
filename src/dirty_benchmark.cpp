...
std::string benchToString(BenchConf c) {
  ...
  res += fmt::format(" READ {:.1f}% WRITE {:.1f}%", 100 * c.readPct,
                     100 * c.writePct);
...
void doBenchInner(BenchConf conf) {
  ...
  if (tracker->getType() != conf.mode) {
    SPDLOG_ERROR("Tracker not expected mode: {} != {}", tracker->getType(),
                 conf.mode);
    exit(1);
  }
  ...
  SPDLOG_INFO("S {:.2f}ms R {:.2f}ms D {:.2f}ms W {:.2f}ms; TOT {:.2f}ms",
              startMillis, runMillis, diffingMillis, writingMillis,
              totalMillis);
  ...
  for (int t = 0; t < conf.nThreads; t++) {
    ...
    int nDirty = std::count(res.dirtyPages.begin(), res.dirtyPages.end(), 1);
    actualDirty += nDirty;

    SPDLOG_TRACE(
        "Thread {} processed {} pages ({} writes, {} reads, {} dirty (TLS))", t,
        res.nPages, res.nWrites, res.nReads, nDirty);

    expectedDirty += res.nWrites;
  }

  if (actualDirty != expectedDirty) {
    SPDLOG_ERROR("FAILED: on {}: {} != {}", conf.mode, actualDirty,
                 expectedDirty);
